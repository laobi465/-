...
export function startScheduler() {
  ...
  // Check GitHub for new commits every 10 seconds so the version badge stays
  // current without needing a page refresh.
  updateTimer = setInterval(() => {
    checkForUpdate().catch((e) =>
      console.warn('[scheduler] update check failed:', (e as Error).message),
    );
