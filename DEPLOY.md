...
> 注意：自动更新依赖容器内的 git 仓库可正常 `pull`（公网仓库无需鉴权）。若修改了远程地址或使用了私有仓库，需确保容器内配置了访问凭证。定时巡检每 10 秒请求一次 GitHub API，匿名接口限流为每小时 60 次——**默认 10 秒间隔会在约 10 分钟内耗尽配额**，仅适合自用监控台；如需降低频率，修改 `server/src/config.ts` 的 `update.checkIntervalMs` 后重新构建。
